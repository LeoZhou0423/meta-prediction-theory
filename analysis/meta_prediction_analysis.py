...
# Load demographics
demographics = pd.read_csv(f"{data_path}/demographics.tsv", sep='\t')
...
# Get BDI item columns (Q1-Q21)
bdi_items = [col for col in bdi_data.columns if col.startswith('Q') and col[1:].isdigit()]
bdi_items = sorted(bdi_items, key=lambda x: int(x[1:]))

# Calculate BDI total score
bdi_data['bdi_total'] = bdi_data[bdi_items].replace(-999, np.nan).sum(axis=1)
...
# This prevents cross-join (Cartesian product) across different visits
...
bdi_responses = bdi_data[bdi_items].replace(-999, np.nan)

# Extreme responding (proportion of 0 and 3 responses)
merged_data['extreme_response'] = bdi_responses.apply(
...
merged_data['midpoint_response'] = bdi_responses.apply(
...
merged_data['response_variability'] = bdi_responses.std(axis=1)

# Missing data proportion
merged_data['missing_prop'] = bdi_responses.isna().mean(axis=1)
...
plt.tight_layout()
plt.savefig(OUTPUT_DIR / "raw_vs_corrected.png", dpi=300, bbox_inches='tight')
plt.close()
...
plt.tight_layout()
plt.savefig(OUTPUT_DIR / "metacognitive_variables.png", dpi=300, bbox_inches='tight')
plt.close()
...
# Save analysis data
merged_data.to_csv(OUTPUT_DIR / "analysis_data.csv", index=False)
