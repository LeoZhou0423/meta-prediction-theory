...
\subsection*{S1. Cross-Scale Generalizability Analysis}

A key concern with the primary analysis is whether the RT-based meta-cognitive proxies capture effects that are specific to the PHQ-9/GAD-7 relationship, or whether they generalize to predicting other psychological constructs. To address this, we used the same proxy variables (DEL, MM, RA) computed from PHQ-9/GAD-7 response times to predict scores on two additional scales in the dataset: the Insomnia Severity Index (ISI; 7 items, measuring sleep disturbance) and the Perceived Stress Scale (PSS; 7 items, measuring perceived stress).

If RT patterns generalize to predicting ISI and PSS scores above and beyond PHQ-9+GAD-7 alone, this would demonstrate that the proxy effects are not specific to the depression-anxiety relationship. Conversely, if the proxies only predict PHQ-9 scores but not ISI or PSS, this would suggest the effects may be specific to the concurrent-validity context.

\begin{table}[h]
\centering
\caption{Cross-Scale Prediction Results}
\label{tab:cross_scale}
\footnotesize
\begin{tabular}{@{}lccccc@{}}
\toprule
\textbf{Target} & \textbf{Model} & \textbf{R$^2$} & \textbf{AIC} & \textbf{dR$^2$} & \textbf{CV-R$^2$} \\
\midrule
\multirow{2}{*}{\textbf{PHQ-9}} 
  & Baseline (GAD-7) & .587 & 111,131 & --- & --- \\
  & Full No-SA & .795 & 94,096 & \textbf{.208} & .794 \\
\midrule
\multirow{2}{*}{\textbf{PSS}} 
  & Baseline (PHQ-9+GAD-7) & .186 & 75,853 & --- & .178 \\
  & Full No-SA & .196 & 75,558 & \textbf{.010} & .187 \\
\midrule
\multirow{2}{*}{\textbf{ISI}} 
  & Baseline (PHQ-9+GAD-7) & .446 & 45,639 & --- & .441 \\
  & Full No-SA & .447 & 45,622 & .001 & .441 \\
\bottomrule
\end{tabular}
\smallskip
\footnotesize \textit{Note.} Full No-SA model includes PHQ-9, GAD-7, DEL, MM, RA as predictors. dR$^2$ = change in R$^2$ from baseline. CV-R$^2$ = mean 5-fold cross-validated R$^2$. All models estimated via OLS ($N = 24{,}292$).
\end{table}

The results (Table~\ref{tab:cross_scale}) show a graded pattern of generalizability. The primary PHQ-9 prediction shows strong effects ($\Delta R^2 = .208$; $\Delta$AIC $= -17{,}035$). The PSS prediction shows a small but meaningful improvement ($\Delta R^2 = .010$; $\Delta$AIC $= -295$; 5-fold CV: $\Delta$CV-R$^2$ $= .009$), driven primarily by the MM (RT variability) proxy (sr$^2$ $= .0098$). The ISI prediction shows negligible improvement ($\Delta R^2 = .0005$; $\Delta$AIC $= -16$).

This graded pattern is interpretable: depression, anxiety, and stress share substantial cognitive-emotional overlap \citep{Clark1991}, so RT patterns associated with depression/anxiety responding partially generalize to stress. Insomnia, by contrast, is more physiological in nature and less directly related to the meta-cognitive processes captured by our proxies. The fact that the proxies generalize selectively---to PSS but not ISI---provides evidence that the effects are not simply noise, but also not universal. This specificity pattern is more informative than either perfect generalizability or complete failure to generalize.
